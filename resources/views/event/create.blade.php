...
<div class="card card-modern">
            ...
            <div class="card-body p-4">
                <form action="{{ route('kelola-event.store') }}" method="POST">
                    @csrf

                    <div class="mb-3">
                        <label class="form-label fw-semibold">Judul Event <span class="text-danger">*</span></label>
                        <input type="text" name="judul" class="form-control @error('judul') is-invalid @enderror"
                               value="{{ old('judul') }}" required placeholder="Contoh: Festival Pasar Terapung 2026">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="id_objek" class="form-select @error('id_objek') is-invalid @enderror">
                            ...
                            @foreach($objekWisatas as $ow)
                                <option value="{{ $ow->id }}" {{ old('id_objek') == $ow->id ? 'selected' : '' }}>
                                    {{ $ow->nama_objek }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="link_url" class="form-control @error('link_url') is-invalid @enderror"
                               value="{{ old('link_url') }}" placeholder="Contoh: link grup WA, poster, atau info lengkap">
                        <small class="text-muted">Kosongkan jika event tidak perlu tautan.</small>
