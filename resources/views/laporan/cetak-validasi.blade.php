...
<head>
    <meta charset="UTF-8">
    <title>Laporan Data Validasi Tiket Gate</title>
    <style>
...
<body onload="window.print()">
    ...
    <h1>Laporan Data Validasi Tiket Gate</h1>
    ...
    <table class="data-table">
        <thead>
            <tr>
                ...
                <th width="13%">Waktu Validasi</th>
                <th>Objek Wisata</th>
                <th width="16%">Kasir/Pengunjung</th>
                <th width="10%">Jml Tiket</th>
            </tr>
        </thead>
        <tbody>
            @php $total_tiket = 0; @endphp
            ...
            <tr>
                ...
                <td class="text-center">{{ date('d/m/y H:i', strtotime($row->waktu_validasi)) }}</td>
                <td>{{ $row->nama_objek ?? '-' }}</td>
                <td>{{ $row->nama_terkait ?? '-' }}</td>
                <td class="text-center">{{ $row->jumlah_tiket }}</td>
            </tr>
            @php $total_tiket += $row->jumlah_tiket; @endphp
            @empty
            <tr><td colspan="7" class="text-center">Tidak ada aktivitas validasi pada periode ini.</td></tr>
            @endforelse
        </tbody>
        <tfoot>
            <tr class="total-row">
                <td colspan="6" class="text-right">TOTAL TIKET TERVALIDASI</td>
                <td class="text-center">{{ $total_tiket }}</td>
            </tr>
        </tfoot>
    </table>

    <p style="font-family: Arial, sans-serif; font-size: 10px; color: #555; margin-top: 8px;">
        Keterangan: Kolom "Kasir/Pengunjung" menampilkan nama kasir untuk transaksi offline, atau nama pengunjung untuk pesanan online.
    </p>
